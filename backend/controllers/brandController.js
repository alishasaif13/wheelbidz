...
export const getModels = async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      `select b.*, m.* from tbl_brands b
        join tbl_model m on 
        b.id = m.brandId WHERE m.status = 'Y' LIMIT ? OFFSET ?`,
      [limit, offset]
    ); 
...
export const deleteModel = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(`SELECT * FROM tbl_model WHERE status = 'N' and id = ?`, [id]);
...
export const addSeries = async (req, res) => {
  try {
    ...
    const [query] = await pool.query(
      "INSERT INTO tbl_series (brandId, modelId, seriesName) VALUES (?, ?, ?)",
      [brandId, modelId, seriesName]
    );

    const insertedId = query.insertId;
    console.log("Inserted series ID:", insertedId);

    // Fetch series with joined brand & model details
    const [result] = await pool.query(
      `select b.*, m.* from tbl_brands b
      join tbl_model m on 
      b.id = m.brandId
...
export const getSeries = async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      `select b.*, m.*, s.* from tbl_brands b
      join tbl_model m on 
      b.id = m.brandId
      join tbl_series s 
      on s.modelId = m.id WHERE s.status = 'Y' LIMIT ? OFFSET ?`,
      [limit, offset]
    ); 
...
export const deleteSereis = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(`SELECT * FROM tbl_series WHERE id = ?`, [id]);
